...
MAX_FRIENDS_THRESHOLD = 200
FRIENDS_OF_FRIENDS_THRESHOLD = 200
...
CONSUMER_SECRET = 'YOUR_CONSUER_SECRET'
...
def get_follower_ids(centre, max_depth=1, current_depth=0, friend_list=[]):

    if current_depth == max_depth:
        print 'out of depth'
        return friend_list

    if centre in friend_list:
        print 'Already been here.'
        return friend_list
    else:
        friend_list.append(centre)
    ...
    try:
            ...
            with open(fname, 'w') as outf:
                ...
                while True:
                    try:
                        ...
                        outf.write('%s\t%s\t%s\n' % params)
                        friend_count += 1
                        if friend_count >= MAX_FRIENDS_THRESHOLD:
        ...
        cd = current_depth
        if cd+1 < max_depth:
            for fid in friendids[:FRIENDS_OF_FRIENDS_THRESHOLD]:
                friend_list = get_follower_ids(fid, max_depth=max_depth,
                    current_depth=cd+1, friend_list=friend_list)

        if cd+1 < max_depth and len(friendids) > FRIENDS_OF_FRIENDS_THRESHOLD:
